...
export default function CodePreview() {
  ...
  useEffect(() => { const timer = setTimeout(() => setShowOutput(true), 2800); return () => clearTimeout(timer); }, []);
      ...
      <div className="bg-gray-900 rounded-2xl shadow-2xl overflow-hidden border border-gray-700">
        ...
        {showOutput && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} transition={{ duration: 0.5 }} className="border-t border-gray-700">
            <div className="flex items-center gap-2 px-4 py-2 bg-gray-800/50"><span className="text-xs text-green-400 font-mono">▶ פלט</span></div>
            <div className="p-4 code-block text-sm bg-gray-950/50">
              {outputLines.map((line, i) => (<motion.div key={i} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: line.delay - 2.8 }} className="text-green-300">{line.text}</motion.div>))}
            </div>
          </motion.div>
        )}
      </div>
